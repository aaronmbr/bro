...
public:
	Connection(NetSessions* s, HashKey* k, double t, const ConnID* id,
	           uint32 flow, int vlan, int inner_vlan, const EncapsulationStack* arg_encap);
	virtual ~Connection();
	...
	bool InnerVLAN(int *ret_vlan) const
		{
		if (inner_vlan == -1)
			return false;
		*ret_vlan = inner_vlan;
...
protected:
	...
	int vlan, inner_vlan; // ethernet VLAN(s)
